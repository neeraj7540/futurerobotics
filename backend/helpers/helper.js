...
module.exports = {
                ...
                if (rawData.hasOwnProperty('imageFolders') && typeof rawData.imageFolders == 'object' && !Array.isArray(rawData.imageFolders) && Object.keys(rawData.imageFolders).length > 0 && Object.keys(req).length > 0) {
                    for (let column in rawData.imageFolders) {
                        ...
                        if (getData.hasOwnProperty(column) && getData[column] != '') {
                            ...
                            getData[column] = `${req.protocol}://${req.get('host')}/${getData[column]}`
                        }
                    }
                }
    ...
    imageUpload: (file, folder = 'users') => {
        ...
        return `public/images/${folder}/${name}`;
    },
    // imageUpload: (file) => {
    //     if (file.name == '') return;
    ...
    sendPushNotification: async function (dataForSend) {
        const serverKey = 'AAAAs4zBDdk:APA91bHK9lCR3q0EDhAqV66ftg08OU9Wtrgd-dVjl3T-1uVBwZaCRbkK145iMf8h8bmDVOy-IBhUM01-IiD80cfXB1d8WrCZBy50DuFq3NuO27SUj2NwBzBx2eSFI7yNHgooJ74IW4vx'; //put your server key here
        const fcm = new FCM(serverKey);

        const title = 'Future Robotics'
