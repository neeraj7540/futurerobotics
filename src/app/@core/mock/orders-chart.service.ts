...
export class OrdersChartService extends OrdersChartData {
  ...
  constructor(private period: PeriodsService, private http: HttpClient) {
    ...
    this.http.get(this.baseUrl + 'usersByDate').subscribe(
      (response: any) => {
        console.log(response.body);
        for (let i = 0; i < 42; i++) {
          this.date.push(response.body[0].dateCreated);
          this.users.push(response.body[0].counted_leads + 170 + i);
        }
        // this.date.push(response.body[0].dateCreated);
        // this.users.push(response.body[0].counted_leads);
